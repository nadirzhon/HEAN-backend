...
class OrderFlowImbalance:
    ...
    async def _handle_tick(self, event: Event) -> None:
        """Handle tick event (trade execution) and publish OFI update."""
        ...
        # Publish OFI update for ContextAggregator
        try:
            ofi_result = self.calculate_ofi(tick.symbol)
            if ofi_result and ofi_result.imbalance_strength > 0:
                await self._bus.publish(Event(
                    event_type=EventType.OFI_UPDATE,
                    data={
                        "symbol": tick.symbol,
                        "ofi_value": ofi_result.ofi_value,
                        "ofi_trend": "bullish" if ofi_result.ofi_value > 0.1 else (
                            "bearish" if ofi_result.ofi_value < -0.1 else "neutral"
                        ),
                        "aggression_buy": ofi_result.buy_pressure,
                        "aggression_sell": ofi_result.sell_pressure,
                        "book_imbalance": ofi_result.imbalance_strength,
                    },
                ))
        except Exception:
            pass  # OFI calculation may fail if no orderbook data yet
    ...
    def _calculate_ofi_python(self, symbol: str) -> OFIResult:
        ...
        for _price, size in bids[:10]:  # Top 10 bid levels
            level_ofi = size / total_volume
            price_level_ofi.append(level_ofi)

        for _price, size in asks[:10]:  # Top 10 ask levels
