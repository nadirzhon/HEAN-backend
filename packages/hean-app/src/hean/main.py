...
class TradingSystem:
    ...
    async def start(self, price_feed=None) -> None:
        ...
        if self._intelligence_gate:
            # IntelligenceGate: SIGNAL → enrich → ENRICHED_SIGNAL → slim handler
    ...
    async def _handle_enriched_signal(self, event: Event) -> None:
        """Risk-First signal handler — slim replacement for _handle_signal_legacy.
    ...
    async def _handle_signal_legacy(self, event: Event) -> None:
        """[LEGACY] Original 700-line signal handler — kept for one release cycle."""
        if self._stop_trading:
