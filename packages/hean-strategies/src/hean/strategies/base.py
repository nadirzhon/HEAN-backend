...
class BaseStrategy(ABC):
    ...
    def __init__(self, strategy_id: str, bus: EventBus) -> None:
        ...
        # Risk-First: cached risk envelope from RiskSentinel
        self._risk_envelope: RiskEnvelope | None = None
    ...
    async def _handle_risk_envelope(self, event: Event) -> None:
        """Cache risk envelope from RiskSentinel (Risk-First architecture)."""
        if not self._running:
            return
        envelope = event.data.get("envelope")
        if envelope:
            self._risk_envelope = envelope
    ...
    async def _handle_tick(self, event: Event) -> None:
        """Handle tick events with Risk-First envelope + SSD lens filtering."""
        if not self._running:
            return

        # === RISK-FIRST: Check cached envelope BEFORE any processing ===
        # This saves CPU by skipping signal generation when risk says "no"
        envelope = self._risk_envelope
        if envelope is not None:
            if not envelope.trading_allowed:
                return  # System-wide halt (killswitch, HARD_STOP, stop_trading)
            if not envelope.can_open_new_position:
                return  # Position/order limits reached
            tick_data_check = event.data.get("tick")
            if tick_data_check and hasattr(tick_data_check, "symbol"):
                if tick_data_check.symbol in envelope.blocked_symbols:
                    return  # Symbol quarantined or already has position
            if envelope.strategy_cooldowns.get(self.strategy_id, False):
                return  # Strategy in cooldown or blocked by protection
        # If envelope is None (RiskSentinel not running): proceed normally (backward compat)
